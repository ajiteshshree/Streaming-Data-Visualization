...
void simulateWeather(double field[][NX][NY], int rank, int num_processes, int numSteps) {
    ...
    if (rank == 0) {
        ...
        // printf("saved\n");
        MPI_Send(tempField, numSteps * NX * NY, MPI_DOUBLE, num_processes-1, 0, MPI_COMM_WORLD);
        printf("Sent from 0\n");
    }
    if(rank == num_processes-1)
    {
        // MPI_Recv(tempField, numSteps * NX * NY, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        // printf("Recieved at n-1\n");
        writeFieldToNetCDF(tempField, numSteps, NX);
        printf("Saved at n-1\n");
    }
}
