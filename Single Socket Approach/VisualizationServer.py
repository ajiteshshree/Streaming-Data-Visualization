...
while True:
        chunk = client_socket.recv(num_steps*NX*NY)
        if not chunk:
            break
        data_bytes += chunk
    
    # Deserialize the bytes to a NumPy array
received_array = np.frombuffer(data_bytes, dtype=np.float64).reshape((num_steps,NX, NY))  # Adjust the shape accordingly
